MePersonalityTagger = function () {
    ...
    this.getMetaTags = function (html, handler) {
        ...
        addWords(getWords(title));
        addWords(getWords(description));
        addWords(keywords, 1);
        var sorted = [];
    ...
    this.getTags3 = function (url, html, text, handler) {
        ...
        var sorted = [];
        for (var word in counter) {
            counter[word] = counter[word] / words.length * .5;
        }
        ...
        MePersonality.tagger.getMetaTags(html,
				function (tags, rels, title, description, keywords) {
				    ...
				    for (var i = 0; i < tags.length; ++i) {
				        if (tags[i] == '') continue;
				        if (counter[escape(tags[i])]) {
				            counter[escape(tags[i])] += rels[i] * .6;
				        }
				        else {
				            counter[escape(tags[i])] = rels[i] * .6;
				        }
        ...
        MePersonality.tagger.textRank(words, function (tags, rels) {
            for (var i = 0; i < tags.length; ++i) {
                if (tags[i] == '') continue;
                if (counter[escape(tags[i])]) {
                    counter[escape(tags[i])] = rels[i] * .5;
                }
                else {
                    counter[escape(tags[i])] = rels[i] * .5;
                }
        ...
        /*
		var len=Math.min(50,sorted.length);
		var eps=1;
		if (len)
			eps=(sorted[0][0]-sorted[len-1][0])/2;//len*10;
		counter={};
		for(var i=0;i<len;++i)
			counter[escape(sorted[i][1])]=sorted[i][0];
		sorted=[];
		var word='';
		words.push('');
		var tags={};
		for(var i=0,rel=0,n=0,last=0;i<words.length;++i){
			var w=escape(words[i]);
			if (counter[w]){
				if (rel&&Math.abs(counter[escape(words[i-1])]-counter[w])<eps){
					//n++; // TODO: temporary
					word+=' '+w;
					rel+=counter[w];
				}
		...
		for(var tag in tags)
			sorted.push([tags[tag],unescape(tag)]);
		sorted.sort(function(a,b){
			if (a[0]!=b[0])
				return b[0]-a[0];
			return a[1]<b[1];
